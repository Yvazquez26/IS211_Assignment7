...
# Assignment 7 - OOP Review and Best Practices
...
class Score(object):
    ...
    def statscurrent(self, player, new_roll):
        print ('%s rolled %s. Score for this turn is %s and player total score is %s' %         (player.name, new_roll, self.total, player.total))

    def playerGreet(self, player):
        print ('%s, current score: %s.' % (player.name,player.total))
...
def main():
    ...
    while players[current_player].total < 100 :
        ...
        if roll == 'r':
            ...
            print(players[current_player].name, 'adds', game.total, 'which gives a grand total of:', 
